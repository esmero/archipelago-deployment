...
docker run -d --name esmero-mc --network="$(basename $(pwd) | tr '[:upper:]' '[:lower:]')_esmero-net" -it --entrypoint sh minio/mc &&
docker network connect "$(basename $(pwd) | tr '[:upper:]' '[:lower:]')_host-net" esmero-mc &&
